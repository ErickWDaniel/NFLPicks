...
class Winners:
    ...
    def __init__(self, database=None):
        ...
        self.picks.loc[:,'winner'] = self.picks['winner'].str.replace('-',' ').str.title()
        self.picks.loc[:,'date'] = pd.to_datetime(self.picks['date'])

        games_rounds = get_games.GetGames().all_games
        self.games_rounds = games_rounds.groupby('check_round').count()[['games']]


        data = get_data.GetData(end_point='results')

        self.results = data.fetch.scrap_result().df
        self.results.loc[:,'game_date'] = pd.to_datetime(self.results['game_date'])

        self.results = self.results[self.results['game_date'] > self.picks['date'].min()]
    ...
    def wins(self, point=1):
        ...
        results.loc[:,'game_round'] = results['game_round'].apply(lambda x: x.split(' ')[-1])
        results.loc[:, 'round_winner'] = results['game_round'] + results['winner']

        results_poll = results['round_winner'].tolist()

        pickers = self.picks[['picker_id','round','winner']]
        pickers.loc[:, 'round_winner'] = pickers['round'].astype(str) + pickers['winner']
        ...
        self.points_per_round = pickers.groupby(['picker_id','round'])['points'].sum().reset_index()
